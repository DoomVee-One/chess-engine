...
var ChessFigure = /** @class */ (function () {
    function ChessFigure(piece, color, x, y) {
        ...
        this.opposing = function (x, y) { return busy(x, y) && getFigure(x, y).color !== _this.color; };
...
var Pawn = /** @class */ (function (_super) {
    ...
    function Pawn(color, x, y) {
        ...
        _this.longMove = function (nx, ny, dx, dy) {
            return !busy(nx, ny) && Math.abs(dy) === 2 && Math.abs(dx) === 0 && (_this.y === 1 || _this.y === 6);
        };
        _this.shortMove = function (nx, ny, dx, dy) {
            return Math.abs(dy) === 1 && ((dx === 0 && !busy(nx, ny)) || (Math.abs(dx) === 1 && _this.opposing(nx, ny)));
        };
        ...
        _this.mockMove = function (nx, ny) {
            ...
            if (dx === 0) {
                for (var i = 1; i < Math.abs(dy); i++) {
                    if (busy(nx, _this.y + k * i))
                        return false;
                }
            }
            else {
                for (var i = 1; i < Math.abs(dx); i++) {
                    if (busy(_this.x + k * i, ny))
                        return false;
                }
            }
            if (busy(nx, ny) && !_this.opposing(nx, ny))
                return false;
            return true;
        };
        return _this;
...
var Knight = /** @class */ (function (_super) {
    ...
    function Knight(color, x, y) {
        ...
        _this.mockMove = function (nx, ny) {
            var dx = nx - _this.x;
            var dy = ny - _this.y;
            return _this.inside(nx, ny) && ((Math.abs(dx) === 2 && Math.abs(dy) === 1) || (Math.abs(dx) === 1 && Math.abs(dy) === 2)) && (!busy(nx, ny) ||
...
function factory(color, piece, x, y) {
    var map = {
        0: Pawn,
        1: Rook,
        2: Knight,
...
var busy = function (x, y) { return (getFigure(x, y) !== undefined); };
var destroy = function (target) { return pieces = pieces.filter(function (piece) { return piece !== target; }); };
;
var getFigure = function (x, y) {
